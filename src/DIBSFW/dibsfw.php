...
require_once dirname(__FILE__) . DS . 'dibs_api' . DS . 'fw' . DS . 'dibs_fw_api.php';

class plgVmPaymentDibsfw extends dibs_fw_api {
    ...
    function __construct(& $subject, $config) {
        ...
        $this->_tableId = 'id';
        $aVarsToPush = array(
            'dibsfw_mid' => array('', 'char'),
            'dibsfw_md5key1' => array('', 'char'),
            
            'dibsfw_md5key2' => array('', 'string'),
            
            
            'dibsfw_cgiapiusr' => array('', 'char'),
            'dibsfw_cgiapipass' => array('', 'char'),
            'dibsfw_testmode' => array('yes', 'char'),
            'dibsfw_fee' => array('no', 'char'),
            'dibsfw_capturenow' => array('no', 'char'),
            'dibsfw_voucher' => array('no', 'char'),
            'dibsfw_uniq' => array('no', 'char'),
            'dibsfw_sendcookies' => array('no', 'char'),
            'dibsfw_color' => array('', 'char'),
            'dibsfw_paytype' => array('', 'char'),
            'dibsfw_paytype' => array('', 'char'),
            'dibsfw_sklspg' => array('', 'char'),
		 
            'dibsfw_lang' => array('da', 'char'),
            'dibsfw_decor' => array('default', 'char'),
            'dibsfw_account' => array('', 'char'),
            'dibsfw_distr' => array('empty', 'char'),
    ...
    function plgVmConfirmedOrder($oCart, $oOrder) {
        ...
        if(empty($aData['merchant'])) {
            JError::raiseWarning(100,JText::_('VMPAYMENT_DIBSFW_MID_NOT_SET'));
            return false;
        }
       
        $sForm = JText::_('VMPAYMENT_DIBSFW_REDIRECT');
    ...
    function plgVmOnPaymentNotification() {
        ...
        $this->method_obj = $method;
        $mOrderInfo = $oModelOrder->getOrder($virtuemart_order_id);
    ...
    $db = JFactory::getDBO();
    $q = 'SELECT * FROM `' . $this->helper_dibs_tools_prefix(). dibs_fw_api::api_dibs_get_tableName() . '` '
