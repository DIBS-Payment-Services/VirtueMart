...
class dibs_fw_api extends dibs_fw_helpers {
    ...
    final public function api_dibs_get_requestFields($mOrderInfo) {
        ...
        $sFingerprint = self::api_dibs_calcHash($aData, 
                                                $this->helper_dibs_tools_conf('md5key1'),
                                                $this->helper_dibs_tools_conf('md5key2'));
    ...
    private function api_dibs_specificFields(&$aData) {
        $ssklspgpage = $this->helper_dibs_tools_conf('sklspg');
        if((string)$ssklspgpage == 'yes') {
    ...
    private function api_dibs_invoiceFields(&$aData, $oOrder) {
        foreach($oOrder->addr as $sKey => $sVal) {
            ...
            $aData ['ordline'.$i.'-2'] = $this->api_dibs_utf8Fix($oItem->name);
            $aData ['ordline'.$i.'-3'] = $this->api_dibs_utf8Fix($oItem->sku);
     ...
     * @return string 
     */
    private static function api_dibs_calcHash($aData, $smd5key1, $smd5key2, $bResponse = FALSE) {
        if($bResponse === TRUE) {
        ...
        return self::api_dibs_calcAnyHash($smd5key1, $smd5key2, $sDataStr);
        
    }
    
    private static function api_dibs_calcAnyHash($smd5key1, $smd5key2, $sDataStr) {
        $smd5key1 = trim($smd5key1, " ,\t,\r,\n");
        $smd5key2 = trim($smd5key2, " ,\t,\r,\n");
        return (!empty($smd5key1) && !empty($smd5key2)) ? md5($smd5key2 . md5($smd5key1 . $sDataStr)) : "";
    }
     ...
     * @return bool 
     */
    private static function api_dibs_checkHash($smd5key1, $smd5key2) {
        return (empty($smd5key1) || empty($smd5key2) || 
               $_POST['authkey'] == self::api_dibs_calcHash($_POST, $smd5key1, $smd5key2, TRUE)) ? 
               TRUE : FALSE;
    }
    ...
    private function api_dibs_cgi_renderControlsBlock($aState, $iTransac) {
        ...
        $sButtonsDecor = (property_exists($this, 'sButtonsClass') && !empty(self::$sButtonsClass)) ?
                     ' class="' . $this->helper_dibs_cgi_getButtonsClass() . '"' : "";
        
        if((int)$aState['status'] == 2 || (int)$aState['status'] == 5) { 
            $sActions = ((int)$aState['status'] ==  2) ?
                        $this->api_dibs_cgi_renderButton('cancel',  $sButtonsDecor) . "&nbsp;" .
                        $this->api_dibs_cgi_renderButton('capture', $sButtonsDecor) :
                        $this->api_dibs_cgi_renderButton('refund',  $sButtonsDecor);
     ...
     * @param string $sButtonsDecor
     * @return string 
     */
    private function api_dibs_cgi_renderButton($sCapture, $sButtonsDecor) {
        return $this->api_dibs_renderTemplate(self::$aTemplates['tmpls']['buttons'] . "ActionsExt", 
                                              array('decorator'   => $sButtonsDecor, 
                                                    'name'        => 'cgi' . $sCapture, 
    ...
    private function api_dibs_cgi_makeRequest($sAuthStr, $sQuery, $aParams) {
        ...
        elseif(function_exists('fsockopen') && is_callable('fsockopen')) {
            return self::api_dibs_cgi_makeRequestSocket($sAuthStr, $sQuery, $aParams);
        }
     ...
     * @return string 
     */
    private static function api_dibs_cgi_makeRequestSocket($sAuthStr, $sQuery, $aParams) {
        $sParams = "";
        $sResult = "";
    ...
    private function api_dibs_cgi_calcHash($aData, $sAPI) {
        $smd5key1 = $this->helper_dibs_tools_conf("md5key1");
        $smd5key2 = $this->helper_dibs_tools_conf("md5key2");
        $sDataStr = ($sAPI == 'capture' || $sAPI == 'refund') ? '&amount=' . $aData['amount'] : "";
        return self::api_dibs_calcAnyHash($smd5key1, $smd5key2, 'merchant=' . $aData['merchant'] . 
